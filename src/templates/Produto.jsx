...
export default function Produto(props) {
    ...
    function adicionarCarrinho(produto) {
        const findProduct = props.listaCarrinho.find(produtFind => produto.id === produtFind.id);
        if (!findProduct) {
        ...
        prods.push(produto);
        localStorage.setItem("lista-produtos", JSON.stringify(prods))
    }

    function removerProduto(produto) {
        if (window.confirm('Deseja realmente excluir esse produto?')) {
            props.setListaCarrinho(
                props.listaCarrinho.filter((item => item.id === produto.id))
            );
        }
    }

    if (props.exibirLista === false) {
        return (
            <div style={{
                width: '200px',
                height: '300px',
        ...
        return (
            <div style={{
                ...
                <div id='botao-comprar'>
                    <button onClick={() => {
                        adicionarCarrinho(props.produto)
                    }}
                        style={{
                            backgroundColor: 'rgb(255,60,60)',
                            color: 'white',
